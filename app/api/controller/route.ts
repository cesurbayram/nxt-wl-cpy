...
export async function POST(request: NextRequest) {
    const { name, model, application, ipAddress, status, serialNumber, intervalMs, maxConnection, location }: Controller = await request.json();
    ...
    try {
        ...
        await client.query(
            ...
            [newRobotId, name, model, application, ipAddress, status, serialNumber, intervalMs, maxConnection, location]
        );
        ...
        await client.query(
            ...
            [newRobotStatusId, ipAddress, newRobotId, 'TEACH', false, false, 'CYCLE', false, false, false, false, false]
        );
...
export async function PUT(request: NextRequest) {
    const { id, name, model, application, ipAddress, status, serialNumber, intervalMs, maxConnection, location }: Controller = await request.json();
    const client = await dbPool.connect();
    try {
        await client.query('BEGIN');
        await client.query(
            ...
            WHERE id = $10`,
            [name, model, application, ipAddress, status, serialNumber, intervalMs, maxConnection, location, id]
        );
        await client.query('COMMIT');
