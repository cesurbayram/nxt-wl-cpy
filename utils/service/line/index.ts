...
const getLine = async (): Promise<Line[]> => {
  ...
  if (apiRes.ok !== true)
    throw new Error("An error occurred when fetching lines");
...
const deleteLine = async ({ id }: Line): Promise<boolean> => {
  ...
  if (apiRes.ok !== true)
    throw new Error("An error occurred when deleting line");
...
const createLine = async (values: Line): Promise<boolean> => {
  ...
  if (!apiRes.ok) {
    ...
    throw new Error(
      `An error occurred when creating line: ${
...
const updateLine = async (values: Line): Promise<boolean> => {
  ...
  if (apiRes.ok !== true)
    throw new Error("An error occurred when updating line.");
