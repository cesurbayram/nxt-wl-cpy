...
const GeneralVariableComponent = ({ controllerId, onMonitoringChange }: GeneralVariableProps) => {
  ...
  const fetchGeneralVariableData = async (isInitialLoad: boolean = false) => {
    try {
      ...
      const typeToUse = isInitialLoad ? 'byte' : (currentVariableTypeRef.current || 'byte');

      if ((isMonitoring && currentVariableTypeRef.current) || isInitialLoad) {
        const data = await getGeneralVariableData(controllerId, typeToUse as GeneralVariableType);

        if (data && data.length > 0) {
          setGeneralVariableData(data);
        } else {
          setGeneralVariableData([]);
        }
      }
