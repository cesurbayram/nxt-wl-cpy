...
} from "@/utils/service/general-data/general-signal";
  ...
  useEffect(() => {
    const initializeComponent = async () => {
      if (controllerId && isFirstRender.current) {
        ...
        await fetchGeneralSignalData(true);
        ...
        try {
          ...
          if (data && data.length > 0) {
            ...
            await sendGeneralSignalCommand(controllerId, firstSignal.general_no);
            currentGeneralNoRef.current = firstSignal.general_no;
            setIsMonitoring(true);
            setGeneralNo(firstSignal.general_no);
            console.log(`Auto-started monitoring General Signal ${firstSignal.general_no}`);
          }
        } catch (error) {
