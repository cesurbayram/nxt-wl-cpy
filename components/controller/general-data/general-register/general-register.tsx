...
} from "@/utils/service/general-data/general-register";
  ...
  useEffect(() => {
    const initializeComponent = async () => {
      if (controllerId && isFirstRender.current) {
        ...
        await fetchGeneralRegisterData(true);
        
        // Eğer kayıtlı veri varsa, ilk kaydı otomatik monitoring yap
        try {
          const data = await getGeneralRegisterData(controllerId);
          if (data && data.length > 0) {
            const firstRegister = data[0];
            await sendGeneralRegisterCommand(controllerId, firstRegister.general_no);
            currentGeneralNoRef.current = firstRegister.general_no;
            setIsMonitoring(true);
            setGeneralNo(firstRegister.general_no);
            console.log(`Auto-started monitoring General Register ${firstRegister.general_no}`);
          }
        } catch (error) {
