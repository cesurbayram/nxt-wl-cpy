...
interface FactoryListProps {
  ...
  const FactoryList = ({ factory, deleteClick }: FactoryListProps) => {
    ...
    const columns: ColumnDef<Factory>[] = [
      {
        accessorKey: "name",
        header: () => (
          <h1 className="text-sm text-[#111827] font-semibold">Factory Name</h1>
        ),
      },
      {
        accessorKey: "status",
          ...
          return (
            <div className="flex justify-center gap-3">
              ...
              <Button
                size="icon"
                variant="ghost"
                onClick={() => router.push(`/factory/${row.original.id}`)}
              >
    ...
    return <DataTable columns={columns} data={factory} />;
  };
